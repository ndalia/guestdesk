...
export function extractFields(message: string, knownFields: Record<string, unknown>) {
  ...
  const party = message.match(/\b(?:party of|table for|for|guests?|people)\s+(\d{1,3})\b/i) ?? message.match(/\b(\d{1,3})\s+(?:people|guests?)\b/i);
  if (party && !fields.partySize) fields.partySize = Number(party[1]);
  if (/\btomorrow\b/i.test(message) && !fields.date) fields.date = "tomorrow";
  const time = message.match(/\b(?:at|around)\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b/i);
  if (time && !fields.preferredTime) fields.preferredTime = `${time[1]}${time[2] ? `:${time[2]}` : ""}${time[3] ? ` ${time[3]}` : ""}`;
  ...
  const name = message.match(/\b(?:my name is|this is|i am|i'm)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\b/);
  if (name && !fields.customerName) fields.customerName = name[1];
...
export function planGuestRequest(runId: string, message: string, knownFields: Record<string, unknown>): ManagerPlan {
  ...
  const wantsReservation = /\b(book|reserve|reservation|table|cancel|change|modify)\b/i.test(message);
  const wantsKnowledge = /\b(close|open|hours?|cake|allerg|parking|transit|address|menu|dress|accessib|policy|bring)\b/i.test(message);
  const wantsCatering = /\b(catering|private event|event|party of \d{2,}|corporate)\b/i.test(message);
  ...
  if (wantsKnowledge) {
    const role = lower.includes("parking") ? "Current Parking Search Specialist" : "Restaurant Knowledge Specialist";
